...
Creating a branch is easy, go to "Source control", click on the three dots next **Changes**, where there should be a dropdown menu with options "Push", "Pull", "Clone",... "Branch". Click on "Branch", and select "Create Branch". Give the branch a name to indicate its not quite ready for production (I typically go with "dev"). Now click on the three dots next to **Changes** again, this time going to "Checkout to...". A dropdown list of branches, including "main" and "dev" should appear. Click on "dev". Now commits will show up on the "dev" branch". The next commit-push will include "Publish branch", which will now be visible on GitHub. 

When you are ready to add the changes in "dev" to the main branch, go to the GitHub page and navigate to "Pull request". If the branch is ahead there should be a highlighted option to merge the branch. Open the pull request, give the merge a name and description, and request the merge. This will go to whomever is the owner of the repo is (on bigger projects pull requests are part of how progress is tracked). If there aren't any issues, there should be a green "No conflicts with base branch." Click it, confirm merge, and now "main" is up to date. This allows for multiple people to work on a project at once, assuming no one is working on the same area of a file at the same time.

## Managing a project on a repository

GitHub isn't a replacement for management. It is a system by which project management can work virtually and in parallel. There are many more useful tools in GitHub (Projects, Codespaces,...) but there still needs to be personel structure that reflects the project and divying of tasks. That is outside the scope of this workshop, but this intro should help with having everyone on a project working at the same level.